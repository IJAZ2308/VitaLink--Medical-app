...
class BedListScreenState extends State<BedListScreen> {
  ...
  final DatabaseReference _bookingRef = FirebaseDatabase.instance.ref().child(
    'bookings',
  );
    ...
    _dbRef.onValue.listen((event) {
      ...
      if (data != null) {
        ...
        hospitalMap.forEach((key, value) {
          ...
          // Calculate available beds based on approved bookings
          int totalBeds = hospital['totalBeds'] ?? 0;
          int bookedBeds = 0;

          if (hospital['bookings'] != null) {
            bookedBeds = (hospital['bookings'] as Map<dynamic, dynamic>).values
                .where((b) => b['status'] == 'approved')
                .length;
          }

          hospital['availableBeds'] = totalBeds - bookedBeds;
          tempHospitals.add(hospital);
        });
      }
  ...
  void _bookBed(Map<String, dynamic> hospital) async {
    int availableBeds = hospital['availableBeds'] ?? 0;
    if (availableBeds <= 0) {
      ScaffoldMessenger.of(
        context,
    ...
    // Push booking request for admin approval
    final bookingRef = _bookingRef.push();
    await bookingRef.set({
      "userId":
          "currentUserId", // Replace with actual FirebaseAuth.currentUser!.uid
      "hospitalId": hospital['id'],
      "status": "pending",
      "timestamp": DateTime.now().millisecondsSinceEpoch,
    });

    // Update hospital bookings node
    final hospitalBookingsRef = _dbRef.child(
      "${hospital['id']}/bookings/${bookingRef.key}",
    );
    await hospitalBookingsRef.set({
      "status": "pending",
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text("Booking requested! Wait for admin approval."),
      ),
    );

    // Navigate to booking screen (optional)
    Navigator.push(
      // ignore: use_build_context_synchronously
      context,
      MaterialPageRoute(
